...
# BERT, BART
    ...
    def load_datasets(self):
        try:
            logging.info(
                "Loading datasets from data ingestion artifacts folder for processing")

            train_dataset = load_from_disk(
                os.path.join(self.dataset_folder, "train"))
            test_dataset = load_from_disk(
                os.path.join(self.dataset_folder, "test"))
    ...
    def preprocess_function(self, examples):
        try:
            ...
            model_inputs = self.tokenizer(
                inputs, max_length=self.config.max_input_length, truncation=True)

            labels = self.tokenizer(
                examples["summary"], max_length=self.config.max_target_length, truncation=True)
    ...
    def initiate_preprocess(self):
        try:
            ...
            logging.error(f"Failed to initiate preprocessing: {e}")
            raise CustomException(e, sys)

    def save_processed_datasets(self, tokenized_train_datasets, tokenized_test_datasets):
        try:
            tokenized_train_datasets.save_to_disk(
                os.path.join(self.config.artifacts_folder, "proc_train"))
            tokenized_test_datasets.save_to_disk(
                os.path.join(self.config.artifacts_folder, "proc_test"))
        except Exception as e:
            logging.error(f"Failed to save processed datasets: {e}")
            raise CustomException(e, sys)

    def process_data(self):
        try:
            logging.info("Data processing component started..")
