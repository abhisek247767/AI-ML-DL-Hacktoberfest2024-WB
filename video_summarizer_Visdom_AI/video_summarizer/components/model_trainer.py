...
class ModelTrainer:
    ...
    def initialize_model_tokenizer(self):
        try:
            logging.info("Initializing model and tokenizer for training...")
            ...
            logging.error(
                f"Exception occurred while initializing model and tokenizer: {e}")
            raise CustomException(e, sys)

    def load_tokenized_datasets(self):
        try:
            logging.info(
                "Loading data from data processing artifacts for training...")

            self.tokenized_train_dataset = load_from_disk(
                os.path.join(self.dataset_folder, "proc_train"))
            self.tokenized_test_dataset = load_from_disk(
                os.path.join(self.dataset_folder, "proc_test"))
        except Exception as e:
            logging.error(
                f"Exception occurred while loading tokenized datasets: {e}")
            raise CustomException(e, sys)
    ...
    def initialize_collator(self):
        try:
            ...
            logging.error(
                f"Exception occurred while initializing data collator: {e}")
            raise CustomException(e, sys)

    def setup_trainer(self):
        try:
            logging.info("setting up model trainer class...")
            batch_size = 64
            ...
            logging.error(f"Exception occurred while setting up trainer: {e}")
            raise CustomException(e, sys)
...
# a,b,c , d,e
            ...
            logging.error(f"Exception occurred while computing metrics: {e}")
            raise CustomException(e, sys)
