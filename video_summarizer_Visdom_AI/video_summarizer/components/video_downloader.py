...
class VideoDownloader:
    ...
    def _download_youtube(self) -> str:
        ...
        try:
            ...
            logging.info(
                f"Youtube Video downloaded to {os.path.join(self.save_path, video.default_filename)}")
    ...
    def _download_other(self) -> str:
        ...
        try:
            ...
            # https://youtube.com/ -> how to get job in data science
            filename = self.url.split("/")[-1]
            with open(os.path.join(self.save_path, f"{filename}"), "wb") as f:
                for chunk in response.iter_content(chunk_size=4096):
                    f.write(chunk)
            logging.info(
                f"Video downloaded to {os.path.join(self.save_path, filename)}")
