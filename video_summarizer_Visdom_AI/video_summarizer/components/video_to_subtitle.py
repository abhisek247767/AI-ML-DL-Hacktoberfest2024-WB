...
# get_subtitles -> Output
    ...
    def extract_audio_from_video(self, video_paths: List[str]) -> Dict[str, str]:
        # tempfile Return the name of the directory used for temporary files
        ...
        for video_path, audio_path in audio_paths.items():
            try:
                ...
                subtitle_path = os.path.join(
                    self.output_dir, f"{filename}.srt")  # abc.srt
                ...
                logging.error(
                    f"Error generating subtitles for {os.path.basename(audio_path)}: {str(e)}")
    ...
    def get_subtitles(
        ...
        subtitles_paths = self.generate_subtitles(
            ...
            lambda audio_path: model.transcribe(
                audio_path, verbose=verbose, task=task),
        )
